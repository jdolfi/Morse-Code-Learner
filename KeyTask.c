...
void KeyTask()
{
	...
	if (SignalIsSet(SIGNAL_KEY_TICK))
	{
		...
		keyTimeoutCount++ ;
		if (keyTimeoutCount > 200) // 2 seconds
		{
			keyTimeoutCount=0;
			
			KeyOff();
			if (keyState != KEY_STATE_MACRO_WAIT)
				keyState = KEY_STATE_IDLE ;			
		}
	}
	...
	switch (keyState)
	{
		...
		case KEY_STATE_MACRO_PLAY:
			if ( latchedKeys || HwButtonsPressed() )
			{
				KeyOff();
				macroReplayCount=0;
				keyState = KEY_STATE_MACRO_ABORT ;
			}
			else if (!elemCount)
			{
				macroReplayCount=0;
				keyState = KEY_STATE_MACRO_WAIT ;
			}			
			else
			{
				unitTickCount = 0;
		...
		case KEY_STATE_MACRO_WAIT:
			...
			else if (!nvSettings[NV_REPLAY])
				keyState = KEY_STATE_IDLE ;
			
			else if (SignalIsSet(SIGNAL_REPLAY_TICK))
			{
				ClearSignal(SIGNAL_REPLAY_TICK);
				macroReplayCount++ ;
		...
		case KEY_STATE_MACRO_ABORT:
			if (SignalIsSet(SIGNAL_REPLAY_TICK))
			{
				ClearSignal(SIGNAL_REPLAY_TICK);				
				macroReplayCount++ ;
				ClearSignal(SIGNAL_PB_ALL);
				// wait second
				if (macroReplayCount > 4 )
				{	
					KeyOff();						
					keyState = KEY_STATE_IDLE ;					
				}
